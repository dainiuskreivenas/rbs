...
for key in rbs.retractions.keys():
    ...
    re.printSpikes("pkls/Monkey_TwoSameFruits/retractions/{}.pkl".format(re.label))

# "################### assertions ###############"

for key in rbs.assertions.keys():
    aa = rbs.assertions[key]
    aa.printSpikes("pkls/Monkey_TwoSameFruits/assertions/{}.pkl".format(aa.label))

# "################### facts ###############"

for key in list(rbs.factGroups):
    for f in rbs.factGroups[key]:
        f[0][1].printSpikes("pkls/Monkey_TwoSameFruits/facts/{}.pkl".format(f[0][1].label))
